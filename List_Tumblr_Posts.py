...
data = client.posts(tumblr_title)

retreived_post_count = len(data['posts'])

# List
# for i in range(0, retreived_post_count):
#     print data['posts'][i]['id']

for i in range(0, retreived_post_count):
    post_id = int(data['posts'][i]['id'])
    print "Deleting post (id: %s)" % post_id
    client.delete_post(tumblr_title, post_id) # Deletes your post :(
